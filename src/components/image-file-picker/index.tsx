...
export const ImagePicker: FC<ImagePickerComponentProps> = forwardRef(function ImagePickerComponent(inputProps, ref) {
  ...
  const handleDeleteFile = async () => {
    ...
    if (filesQuery?.files?.[0]) {
      await deleteFileAssociation(authorizedFetcher, filesQuery?.files?.[0].id);
    }
    mutate();
  };
  ...
  const handleUpload = useCallback(async () => {
    if (selectedImage) {
      ...
      await createFileAssociation(authorizedFetcher, {
        entityId,
        entity,
        fileId: file.fileId,
  ...
  }, [authorizedFetcher, fileUploader, selectedImage, entity, entityId, filesQuery]);
